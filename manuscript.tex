...
Rather than aggregating participation into summary counts, examining separate binary items simultaneously allows researchers to observe whether taste change is uniform across an entire expressive sphere or driven by specific, institutionalized activities. Using longitudinal survey data from the NetSense study, this investigation models individual developmental pathways across three expressive domains: public arts and cultural event attendance (tracked across 9 distinct events from freshman through sophomore year), private leisure reading choices (tracked across 9 distinct book types from freshman through junior year), and musical genre preferences (tracked across 10 prominent musical genres from freshman through junior year). We implement a multivariate binary latent class growth model using natural cubic splines in the \texttt{flexmix} package \citep{grun2008flexmix} in \textsf{R} \citep{Rmanual}. This approach models individual-level trajectories for each item simultaneously while estimating endogenous multinomial concomitants to identify how sociodemographic background, academic achievement, and collegiate major structure latent class membership.
...
Across all three expressive domains, cultural items are measured as binary indicators ($Y_{itj} \in \{0, 1\}$) reflecting semester-level participation or aesthetic preference. For public arts and cultural events, tracked across Waves 1 through 4, participation was measured by survey question Q45: ``\textit{In the past semester, have you attended any of the following cultural events?}'' Respondents indicated binary participation (``Yes'' = 1, ``No'' = 0). To focus specifically on aesthetic and expressive arts while consolidating conceptually aligned genres, we combine classical concerts and opera into a single category, combine rock/pop and folk/country concerts into a single live music category, and eliminate science and history museums. This yields nine distinct cultural activities spanning institutional highbrow, theatrical, and commercial popular venues: classical music concerts or opera; rock, pop, folk, or country concerts; ballet or modern dance; jazz or blues performances; musical theater or Broadway shows; stage plays or dramas (non-musical); comedy clubs or stand-up shows; art museums or galleries; and movies in a cinema or theater.
...
\addlinespace[3pt]
\multicolumn{4}{l}{\textbf{Panel B: Public Arts \& Cultural Events (9 Items, Waves 1--4, $N = 199$)}} \\
\quad Stage Play (Non-Musical) & Attended in past semester & 43.1\% & W1: 58.0\% $\rightarrow$ W4: 47.8\% \\
\quad Comedy Club & Attended in past semester & 39.9\% & W1: 56.9\% $\rightarrow$ W4: 39.8\% \\
\quad Ballet or Modern Dance & Attended in past semester & 38.1\% & W1: 67.4\% $\rightarrow$ W4: 35.4\% \\
\quad Rock, Pop, Folk, or Country & Attended in past semester & 33.9\% & W1: 40.9\% $\rightarrow$ W4: 31.1\% \\
\quad Jazz or Blues Performance & Attended in past semester & 33.6\% & W1: 53.0\% $\rightarrow$ W4: 23.6\% \\
\quad Art Museum or Gallery & Attended in past semester & 27.8\% & W1: 53.6\% $\rightarrow$ W4: 28.6\% \\
\quad Cinema or Movie Theater & Attended in past semester & 23.4\% & W1: 34.8\% $\rightarrow$ W4: 29.8\% \\
\quad Classical Concert or Opera & Attended in past semester & 20.8\% & W1: 17.7\% $\rightarrow$ W4: 25.5\% \\
\quad Musical Stage Play & Attended in past semester & 15.0\% & W1: 18.8\% $\rightarrow$ W4: 14.9\% \\
\addlinespace[3pt]
...
\midrule
\multicolumn{8}{l}{\textbf{Panel A: Arts \& Cultural Events (9 Items, Waves 1--4, $N = 199$)}} \\
$K = 1$ & -3334.9 & 27 & 6723.8 & 6844.8 & --- & --- & 100\% \\
$K = 2$ & -3048.3 & 55 & 6206.6 & 6453.2 & 573.2 (28) & $< .001$ & 65.8\% / 34.2\% \\
$K = 3$ & -2990.4 & 83 & 6146.8 & 6518.9 & 115.8 (28) & $< .001$ & 19.1\% / 35.2\% / 45.7\% \\
$K = 4$ & -2938.7 & 111 & 6099.3 & 6597.0 & 103.5 (28) & $< .001$ & 46.7\% / 17.6\% / 9.5\% / 26.1\% \\
$K = 5$ & -2958.8 & 139 & 6139.6 & 6637.2 & --- & --- & 38.7\% / 20.6\% / 21.6\% / 19.1\% \\
\addlinespace[3pt]
...
Across all three domains, moving from a single homogeneous growth curve ($K = 1$) to a multi-class mixture produces marked improvements in predictive log-likelihood. In leisure reading and music preferences, the three-class specification captures the primary structural elbow: moving from two to three classes yields large reductions in BIC \citep{raftery1995bayesian} ($\Delta\text{BIC} = -340.3$ in Books; $\Delta\text{BIC} = -241.4$ in Music; $p < 0.001$), generating well-populated, substantively cohesive groups of roughly equal size ($25\%$ to $45\%$). For public cultural events, estimating 9 item trajectories consumes 28 additional parameters per class; while formal BIC reaches its minimum at $K = 2$ due to heavy dimensionality penalization, the stepwise Likelihood Ratio Test indicates that $K = 3$ improves model fit decisively ($\chi^2 = 115.8, df = 28, p < 0.001$). Substantively, $K = 3$ is essential: it cleanly separates high-arts traditionalists from broader cultural consumers, a distinction that two-class models obscure. Beyond three classes, solutions fragment existing classes into redundant sub-clusters. Consequently, $K = 3$ is selected as the primary substantive specification across all three domains.

\section{Item-Specific Trajectory Profiles}

\subsection{Public Arts and Cultural Events (9 Items)}

Figure~\ref{fig:arts} displays the model-implied predicted participation trajectories across all nine cultural event types for each of the three latent classes over the first four collegiate semesters. Lines trace continuous natural cubic spline curves, while discrete geometric markers designate empirical survey wave observations. All trajectory profiles and small-multiple panels were visualized using the \texttt{ggplot2} package \citep{wickham2016ggplot2} in \textsf{R} \citep{Rmanual}.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{Plots/fig1_arts_9_events_by_class.png}
\caption{Latent participation trajectories across 9 Public Arts and Cultural Events from the 3-class multivariate binary model (Waves 1 to 4). Circles and blue curves denote Class 1 (``Omnivores''), triangles and orange curves denote Class 2 (``Traditionalists''), and squares and sky blue curves denote Class 3 (``Minimalists'').}
\label{fig:arts}
\end{figure}

Examining the item-specific trajectories shows how the collegiate sophomore slump operates differently across distinct cultural activities. The first class, designated ``Class 1: Omnivores'' ($n = 38$, 19.1\%), maintains elevated attendance across all nine expressive activities. Members of this class enter college with high engagement across ballet and modern dance (95.6\%), comedy clubs (91.2\%), non-musical stage plays (87.2\%), rock, pop, folk, or country concerts (81.5\%), jazz or blues (80.7\%), cinema (78.2\%), and art museums (72.7\%). While experiencing a modest mid-collegiate dip in popular venues, their participation rebounds by sophomore spring. Moreover, classical music concert and opera attendance among Omnivores rises monotonically from 49.8\% to 77.6\% (+27.8 percentage points) as students cultivate specialized highbrow cultural capital. The second group, ``Class 2: Traditionalists'' ($n = 70$, 35.2\%), concentrates its cultural capital on institutionalized highbrow and theatrical venues, participating actively in comedy clubs (74.6\%), art museums (65.4\%), ballet (65.1\%), and stage plays (60.9\%). However, Traditionalists experience deep sophomore slumps in museum and comedy attendance (falling by 42.8 percentage points in art museums and 40.1 percentage points in comedy clubs) before staging partial rebounds. The third group, ``Class 3: Minimalists'' ($n = 91$, 45.7\%), shows universally lower attendance across all nine activities, with participation in rock and folk concerts dropping from 21.4\% to 2.9\%, jazz dropping from 37.9\% to 7.0\%, and musical theater falling to 3.4\% by sophomore year.
...
\midrule
\multicolumn{7}{l}{\textbf{Panel A: Arts \& Cultural Events (9 Items, $N = 199$)}} \\
\quad Block 1: Demographic Identity & 10.69 & 16.71 & 6 & 0.010 & 413.8 & 440.1 \\
\quad Block 2: Family SES & 3.75 & 3.75 & 4 & 0.440 & 422.8 & 442.5 \\
\quad Block 3: Scholastic Capital & 20.15 & 3.80 & 4 & 0.433 & 422.7 & 442.5 \\
\quad Block 4: Collegiate Context & 5.87 & 5.87 & 4 & 0.209 & 420.6 & 440.4 \\
\quad Combined Sociodemographics (Blocks 1+2) & 20.13 & 20.13 & 10 & 0.028 & 418.4 & 457.9 \\
\quad Full Multivariable Model & 27.72 & 27.72 & 18 & 0.066 & 426.8 & 492.7 \\
\addlinespace[3pt]
...
The block tests provide compelling evidence regarding the social stratification of cultural trajectories. Demographic identity (Gender, Race, and Religion) significantly predicts class sorting across all three domains: public arts ($\chi^2 = 16.71, df = 6, p = 0.010$), leisure reading ($\chi^2 = 94.64, df = 6, p < 0.001$), and music preferences ($\chi^2 = 30.62, df = 6, p < 0.001$). Pre-collegiate scholastic capital (GPA and degree aspirations) is highly significant in music ($\chi^2 = 19.72, df = 4, p < 0.001$). Collegiate context (STEM major and hometown urbanicity) significantly predicts class membership in both books ($\chi^2 = 11.43, df = 4, p = 0.022$) and music ($\chi^2 = 13.92, df = 4, p = 0.008$). In contrast, family socioeconomic status (parental income and education) is completely non-significant across all three domains (e.g., $\chi^2 = 3.75, p = 0.440$ in arts; $\chi^2 = 0.89, p = 0.926$ in books; $\chi^2 = 3.70, p = 0.448$ in music). Combined sociodemographics achieve decisive significance in both books and music ($p < 0.001$) and in arts ($p = 0.028$).

\subsection{Model-Implied Marginal Class Probabilities}

While Table~\ref{tab:blocks} evaluates the overall predictive power of theoretical variable blocks, interpreting individual coefficients from multinomial logit models is often complicated by arbitrary reference baselines and non-linear log-odds metrics. Table~\ref{tab:app_coefs} in the Appendix reports the complete multivariable parameter estimates and odds ratios for all nine covariates across all three domains. Following recent methodological practice in latent growth mixture analysis, we translate these multivariable model parameters into model-implied marginal predicted class probabilities with 95\% simulation confidence intervals, holding other background covariates constant at sample means and reference categories. Figure~\ref{fig:marginal} illustrates these marginal class probabilities across the statistically significant predictor blocks identified in Table~\ref{tab:blocks} for each of the three expressive domains.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{Plots/fig4_marginal_effects_all_domains.png}
\caption{Model-implied marginal predicted trajectory class probabilities with 95\% simulation confidence intervals across statistically significant predictor blocks from Table~\ref{tab:blocks}. Panel A displays predicted probabilities for Public Arts and Cultural Events across Gender Identity and Degree Aspirations. Panel B displays predicted probabilities for Leisure Book Reading Types across Gender Identity, Religion, and Undergraduate Major. Panel C displays predicted probabilities for Musical Genre Preferences across High School Academic Grades, Gender Identity, and Undergraduate Major. Dashed vertical reference lines indicate the equal random allocation baseline ($33.3\%$).}
\label{fig:marginal}
\end{figure}

In Panel A, Public Arts and Cultural Events displays substantial sorting across demographic identity and pre-collegiate aspirations. Women have a 56.0\% probability (95\% CI: [39.4\%, 70.8\%]) of belonging to Class 2 Traditionalists and a 25.9\% probability of belonging to Class 1 Omnivores, compared to only an 18.2\% probability of sorting into Class 3 Minimalists. In contrast, men are more than twice as likely as women to enter the Minimalist attendance pathway (41.1\%, 95\% CI: [26.7\%, 56.7\%]). Furthermore, educational aspirations exert a substantial extensive-margin sorting effect: students aspiring only to a bachelor's degree have an 83.9\% probability of entering the Minimalist pathway, whereas students planning post-graduate study (Master's, Ph.D., M.D., or J.D.) have only a 41.1\% probability of minimal attendance and are over four times more likely to sort into Traditionalist high-arts attendance (44.1\% vs. 10.7\%).

In Panel B, Leisure Book Reading Types reveals that genre specializations are sharply divided by religious affiliation, gender identity, and undergraduate field of study. Religious identity produces pronounced sorting into reading styles: students identifying as Roman Catholic have a 31.6\% probability of belonging to Class 1 Nonfictionists and a 41.8\% probability of Class 2 Fictionists, with only a 26.6\% probability of sorting into Minimalist reading. Conversely, non-Catholic students are predominantly channeled into Class 3 Minimalist reading (58.1\%, 95\% CI: [37.2\%, 76.0\%]). Collegiate major also sorts students into distinct literary spheres: declared STEM majors exhibit a 59.9\% probability of belonging to Class 2 Fictionists (specializing in science fiction, fantasy, and thrillers) compared to 41.8\% among non-STEM students, alongside lower disengagement (14.9\% vs. 26.6\% for Minimalists).

In Panel C, Musical Genre Preferences illustrates the strong sorting power of pre-collegiate scholastic capital alongside gender identity and academic context. Earning mostly A/A- grades in high school serves as a primary sorting gateway into musical omnivorousness: high-achieving students have a 44.5\% probability of entering Class 1 Omnivores, compared to only 12.3\% (95\% CI: [3.7\%, 28.1\%]) among students with B+ or lower grades. Students entering with lower high school performance sort heavily into Class 3 Mainstreamers (47.2\%) and Class 2 Rockers (40.5\%). Gender identity sharply differentiates the rock subculture: men have a 25.0\% probability of sorting into Class 2 Rockers, whereas women have only a 7.4\% probability (95\% CI: [2.8\%, 15.6\%]), concentrating instead in Class 1 Omnivores (63.8\%). Finally, undergraduate major reinforces these aesthetic boundaries: declared STEM majors are significantly more likely to enter the Rocker pathway (44.2\%, 95\% CI: [31.3\%, 58.0\%]) than non-STEM majors (25.0\%), showing that collegiate fields of study cultivate distinct musical taste cultures.

Across all three domains, parental socioeconomic status (income and parental education) displays negligible direct sorting power once prior academic achievement and demographic characteristics are controlled. Crucially, demographic characteristics operate almost exclusively as between-class sorting gates (the extensive margin) rather than within-class slope modifiers (the intensive margin).\footnote{To test whether baseline characteristics moderate the rate and shape of change within established pathways, we estimated class-stratified multilevel mixed-effects logistic models using the \texttt{lme4} package \citep{bates2015fitting} in \textsf{R} \citep{Rmanual} across all three Arts classes, entering individual random intercepts and evaluating interactions between the time spline basis and focal demographic predictors ($\text{Time} \times \text{Woman}$ and $\text{Time} \times \text{STEM Major}$). Across all three strata, within-class demographic slope moderation is statistically null ($p > 0.10$). Specifically, interaction terms between time and gender identity fail to achieve statistical significance in Class 1 Omnivores ($\beta = +0.295, \text{SE} = 0.229, z = 1.29, p = 0.198$), Class 2 Traditionalists ($\beta = +0.179, \text{SE} = 0.390, z = 0.46, p = 0.647$), and Class 3 Minimalists ($\beta = +0.034, \text{SE} = 0.447, z = 0.08, p = 0.939$). Similarly, interaction terms between time and STEM major are uniformly non-significant in Class 1 ($\beta = +0.178, \text{SE} = 0.232, z = 0.77, p = 0.442$), Class 2 ($\beta = +0.607, \text{SE} = 0.383, z = 1.59, p = 0.113$), and Class 3 ($\beta = -0.111, \text{SE} = 0.440, z = -0.25, p = 0.801$). Once students enter a given developmental corridor, they share a common institutional habitus, experiencing virtually identical sophomore slump and recovery dynamics regardless of their demographic background.}
...
Table~\ref{tab:app_coefs} presents the complete parameter estimates, standard errors, and odds ratios from the full multivariable endogenous multinomial logit models estimated jointly with item trajectories across all three cultural taste domains.
...
Constant & 6.40*** (1.87) & 603.71 & 13.42*** (1.64) & 674218 \\
Woman (ref: Man) & 0.20 (0.46) & 1.22 & -1.47** (0.45) & 0.23 \\
White (ref: Non-White) & 0.33 (0.58) & 1.39 & 0.31 (0.59) & 1.36 \\
Catholic (ref: Non-Catholic) & 0.52 (0.48) & 1.68 & 0.43 (0.48) & 1.53 \\
Parent Income (\$1,000s) & 0.00 (0.00) & 1.00 & 0.00 (0.00) & 1.00 \\
Parent Education (Years) & 0.21 (0.13) & 1.23 & 0.05 (0.12) & 1.05 \\
High School GPA (A/A-) & -0.59 (0.56) & 0.55 & -0.70 (0.56) & 0.49 \\
Aspires to Advanced Degree & -12.58*** (1.22) & 0.00 & -13.91*** (1.21) & 0.00 \\
STEM Major (ref: Non-STEM) & 0.48 (0.46) & 1.61 & 0.35 (0.45) & 1.41 \\
Hometown Urbanicity & 0.53 (0.30) & 1.70 & 0.16 (0.26) & 1.17 \\
\addlinespace[3pt]
