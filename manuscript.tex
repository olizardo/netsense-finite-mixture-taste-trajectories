...
$K = 1$ (Homogeneous Growth) & -4704.3 & 6 & 9420.5 & 9450.0 & $\Delta\text{BIC} = 915.7$ & --- \\
$K = 2$ & -4289.0 & 13 & 8604.1 & 8667.9 & $\Delta\text{BIC} = 133.6$ & $< .001$ \\
$K = 3$ (Optimal Specification) & -4198.0 & 20 & 8436.1 & 8534.3 & $\Delta\text{BIC} = 0.0$ & $< .001$ \\
$K = 4$ & -4180.3 & 27 & 8414.6 & 8547.2 & $\Delta\text{BIC} = 12.9$ & $< .001$ \\
$K = 5$ & -4154.7 & 34 & 8377.3 & 8544.4 & $\Delta\text{BIC} = 10.1$ & $< .001$ \\
\addlinespace[3pt]
\multicolumn{7}{l}{\textbf{Panel B: Predictive Power of Theoretical Variable Blocks ($K = 3$)}} \\
Baseline Specification (Null) & -4198.0 & 20 & 8436.1 & 8534.3 & --- & --- \\
\quad Block 1: Demographic Identity & -4195.2 & 26 & 8442.4 & 8570.2 & 5.64 (6) & 0.465 \\
\quad Block 2: Family SES & -4197.1 & 24 & 8442.2 & 8560.1 & 1.90 (4) & 0.754 \\
\quad Block 3: Scholastic Capital & -4189.6 & 24 & 8427.2 & 8545.1 & 16.90 (4) & 0.002 \\
\quad Block 4: Collegiate Context & -4194.9 & 24 & 8437.8 & 8555.8 & 6.22 (4) & 0.183 \\
\quad Combined Sociodemographics (Blocks 1+2) & -4193.3 & 30 & 8446.7 & 8594.1 & 9.39 (10) & 0.495 \\
\quad Full Multivariable Model & -4181.2 & 38 & 8438.3 & 8625.1 & 33.71 (18) & 0.014 \\
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
\vspace{4pt}
\footnotesize
\textit{Note:} Models estimated in \texttt{flexmix} using joint Poisson distributions parameterized via natural cubic splines with 2 degrees of freedom (\texttt{splines::ns(time, df = 2)}). In Panel A, $\Delta\text{BIC}$ is evaluated relative to the optimal $K = 3$ solution; $p$-values reflect stepwise likelihood ratio tests against $K - 1$. In Panel B, LRT $\chi^2$ evaluates improvement over the null baseline concomitant model. Block 1 includes Gender Identity, Racial Identity, and Religion; Block 2 includes Parental Income and Parental Education; Block 3 includes High School GPA and Advanced Degree Aspirations; Block 4 includes Academic Major (STEM) and Hometown Urbanicity.
\end{minipage}
\end{table}

Table~\ref{tab:bivariate} summarizes the latent class enumeration results in Panel A and theoretical block tests in Panel B. Moving from a single homogeneous growth curve ($K = 1$) to a multi-class mixture produces substantial improvements in model fit. The Bayesian Information Criterion decisively selects a three-class solution ($K = 3, \text{BIC} = 8534.3, \Delta\text{BIC} = 0.0$), outperforming the two-class model by 133.6 BIC points. Adding a fourth class increases BIC ($\Delta\text{BIC} = 12.9$), confirming $K = 3$ as the optimal specification.\footnote{Estimating the bivariate Poisson model using the focal top-10 music genres alongside the 9 book reading genres produces an identical 3-class selection ($\Delta\text{BIC} = 0.0$ at $K = 3$), confirming that the tripartite developmental structure is insensitive to whether musical breadth is measured across all 22 genres or the top 10 genres.}
...
\caption{Bivariate latent trajectories of cultural omnivorousness across college from the 3-class Poisson growth mixture model (Waves 1 to 6, $N = 201$). Blue solid curves and circular markers denote Musical Omnivorousness (total genre count endorsed out of 22 items); orange dashed curves and triangular markers denote Literature Omnivorousness (total book genres read for leisure out of 9 items). Shaded ribbons indicate 95\% confidence intervals around empirical wave means.}
\label{fig:bivariate}
\end{figure}
...
First, ``Class 1: High Dual Omnivores'' ($n = 31, 15.4\%$) displays high cultural volume across both spheres throughout college. In music, their repertoire breadth begins at an average of 10.00 genres in freshman fall, expands to 11.96 genres by sophomore fall, and remains sustained above 11.14 genres through senior spring. In leisure reading, they maintain an average of 4.14 to 4.94 book genres across all semesters. These students embody dual cultural omnivorousness, actively cultivating broad, diverse repertoires in both auditory and literary media.

Second, ``Class 2: Moderate Eclectics'' ($n = 96, 47.8\%$) represents the modal undergraduate pathway. These students maintain a stable, moderate breadth across college, consistently endorsing between 5.44 and 6.21 musical genres and reading between 3.43 and 3.76 book categories across all six semesters. Their trajectories exhibit minimal fluctuation over time, showing neither expansive aesthetic broadening nor winnowing.

Third, ``Class 3: Cultural Minimalists'' ($n = 74, 36.8\%$) enters college with modest repertoires (averaging 3.35 musical genres and 3.35 book genres) and undergoes progressive cultural winnowing across emerging adulthood. By senior year, their musical breadth contracts by 32.2\% (falling to 2.27 genres), while their leisure reading contracts by 21.1\% (falling to 2.65 genres). Rather than expanding their cultural horizons, these students narrow their expressive engagement as collegiate academic demands accumulate.

Panel B of Table~\ref{tab:bivariate} evaluates the predictive power of theoretical variable blocks in sorting students into these bivariate pathways. Pre-collegiate scholastic capital (High School GPA and advanced degree aspirations) emerges as the primary sorting engine ($\chi^2 = 16.90, df = 4, p = 0.002$), significantly lowering AIC from 8436.1 to 8427.2. In contrast, family socioeconomic status (parental income and education) is completely non-significant ($\chi^2 = 1.90, df = 4, p = 0.754$). When all covariates are entered simultaneously, the full multivariable model achieves decisive statistical significance ($\chi^2 = 33.71, df = 18, p = 0.014$).

As detailed in Appendix Table~\ref{tab:app_coefs} (Panel C), multivariable parameter estimates indicate that entering college with high school academic achievement (mostly A/A- grades) acts as a primary protective barrier against cultural disengagement. Students with high prior GPA are nearly 80\% less likely to sort into the Cultural Minimalist pathway ($\text{OR} = 0.206, p = 0.010$) relative to the High Dual Omnivore pathway. Inverted, high scholastic capital increases the odds of sorting into the High Dual Omnivore trajectory by nearly five-fold ($\text{OR} = 4.85, p = 0.010$). Roman Catholic religious affiliation also reduces sorting into Minimalist reading and listening ($\text{OR} = 0.367, p = 0.072$). Finally, the statistical null effect of parental income and education persists in the bivariate count model, reinforcing our earlier qualification that upstream institutional admissions filters at selective universities compress family socioeconomic variance, leaving prior scholastic achievement and campus context as the primary observable sorting mechanisms.
...
Constant & 5.31 (3.37) & 201.81 & 5.89 (3.52) & 360.66 \\
Woman (ref: Man) & 0.12 (0.44) & 1.13 & -0.30 (0.47) & 0.74 \\
White (ref: Non-White) & 0.31 (0.57) & 1.36 & 0.31 (0.60) & 1.36 \\
Catholic (ref: Non-Catholic) & -0.85 (0.54) & 0.43 & -1.00 (0.56) & 0.37 \\
Parent Income (\$1,000s) & 0.00 (0.00) & 1.00 & 0.00 (0.00) & 1.00 \\
Parent Education (Years) & 0.03 (0.12) & 1.03 & -0.04 (0.13) & 0.96 \\
High School GPA (A/A-) & -0.65 (0.62) & 0.52 & -1.58* (0.62) & 0.21 \\
Aspires to Advanced Degree & -0.88 (1.11) & 0.42 & -1.57 (1.11) & 0.21 \\
STEM Major (ref: Non-STEM) & 0.40 (0.44) & 1.49 & 0.32 (0.47) & 1.37 \\
Hometown Urbanicity & -0.58 (0.38) & 0.56 & -0.27 (0.40) & 0.77 \\
\bottomrule
\end{tabular*}
\begin{minipage}{\linewidth}
\vspace{4pt}
