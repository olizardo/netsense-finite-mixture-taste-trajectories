# NetSense Taste Trajectories - AI Agents Log

## Project Goal
Study cultural taste change trajectories in the NetSense dataset using finite mixture models.

## Tasks Completed
- **Project Structure**: Initialized an RStudio `.Rproj` file for standard environment handling.
- **Data Exploration**: Verified the presence of clean data files (`demographics_longitudinal_clean.rds` and `network_surveys_longitudinal_clean.rds`). Checked longitudinal variables for music (`musicpref[1-22]_[1-6]`) and books (`typebookread[1-9]_[1-6]`).
- **Data Prep Script**: Created `01_data_prep_and_fmm.R` to reshape the wide data format into a tidy, longitudinal format, transforming `Yes`/`No` strings into binary format suitable for modeling.
- **Reproducibility**: Initialized `renv` environment. Basic data manipulation packages (`dplyr`, `tidyr`, `stringr`) are logged for installation.
- **Advanced Modeling**: Set up a Bayesian Growth Mixture Model (GMM) with crossed random effects for cultural participation using `brms` and `cmdstanr` (`04_brms_gmm_crossed.R`), optimized for 16 CPU cores.
- **Methodological Pivot (Beta-Binomial Person-Level LCGA)**:
  - Overcame prior `brms` observation-level mixture splitting and continuous random slope variance absorption by implementing a custom Bayesian Latent Class Growth Model in Stan (`Scripts/stan/lcga_betabinomial.stan`).
  - Implemented person-level likelihood accumulation $\prod_t p(Y_{it} \mid c_i = k)$, native ordered intercepts to prevent label switching, and regularizing Dirichlet priors ($\alpha_k = 3.0$) preventing degenerate clusters.
...
- **Table 4**: Bivariate Poisson Latent Class Growth Models of Expressive Omnivorousness ($N = 201$) (`tab:bivariate`)
- **Table A1**: Multinomial Logistic Parameter Estimates and Odds Ratios from Item-Level Multivariate Binary Models (Appendix, Books and Music) (`tab:app_coefs`)
- **Table A2**: Multinomial Logistic Parameter Estimates and Odds Ratios from the Four-Class Bivariate Expressive Omnivorousness Model (Appendix) (`tab:app_coefs_biv`)
...
- **Figure 5**: Bivariate latent trajectories of expressive omnivorousness across college from the 4-class Poisson growth mixture model (`Plots/fig5_bivariate_omnivorousness_trajectories.png`, `fig:bivariate`)
